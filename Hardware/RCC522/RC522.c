...
unsigned char ReadRawRC(unsigned char Address)
{
	...
	Delay_ms(1);
	RC522_ENABLE;
	SPI2_WriteNBytes(&ucAddr,1);  //向总线写多个数据
	SPI2_ReadNBytes(&ucResult,1);  //向总线读多个数据
	RC522_DISABLE;
	return ucResult;
}
...
{  
	...
	Delay_ms(1);
	RC522_ENABLE;
	SPI2_WriteNBytes(write_buffer,2);
	RC522_DISABLE;
}
...
void NFCMatch()
{
	...
	switch(cardType)
	{
			case 0x4400:
					OLED_Clear();
					OLED_ShowString(4,0,"Mifare UltraLight\r\n",OLED_8X16);
					OLED_Update();
					break;
			case 0x0400:
					OLED_Clear();
					OLED_ShowString(4,0,"Mifare S50",OLED_8X16);
					OLED_Update();
					break;
			case 0x0200:
					OLED_Clear();
					OLED_ShowString(4,0,"Mifare S70",OLED_8X16);
					OLED_Update();
					break;
			case 0x0800:
					OLED_Clear();
					OLED_ShowString(4,0,"Mifare ProX",OLED_8X16);
					OLED_Update();
					break;
			case 0x4403:
					OLED_Clear();
					OLED_ShowString(4,0,"Mifare DESFire",OLED_8X16);
					OLED_Update();
					break;
			default:
					OLED_Clear();
					OLED_ShowString(4,0,"Unknown Card",OLED_8X16);
					OLED_Update();
					break;
			}
	if(cardType==0x0400||cardType==0x0200)
	{
				LED1_OFF();
				LED2_OFF();
				LED3_ON();
				...
				OLED_Update();
				Delay_ms(1000);
				while(MI_OK==PcdRequest(0x52, Card_Type1));
	}
	else
	{
				LED2_OFF();
				...
				LED1_ON();
				OLED_Clear();
				OLED_ShowString(8, 0, "Fault Card!", OLED_8X16);
				OLED_Update();
				Delay_ms(1000);
				while(MI_OK==PcdRequest(0x52, Card_Type1));
	}
}
void PcdResetON(void)
{
	PcdReset();  //复位RC522
	PcdAntennaOff();  //关闭天线
	Delay_ms(100);
